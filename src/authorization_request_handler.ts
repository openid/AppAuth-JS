...
export abstract class AuthorizationRequestHandler {
   ...
   */
  completeAuthorizationRequestIfPossible(useHash: boolean = true): Promise<void> {
    ...
    return this.completeAuthorizationRequest(useHash).then(result => {
      if (!result) {
   ...
   * if not ready.
   */
  protected abstract completeAuthorizationRequest(useHash?: boolean):
