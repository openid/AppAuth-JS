...
export class MedblocksAuth {
    ...
    public authListner: AuthorizationListener = async (req, res) => {
        if (res && req.internal) {
            ...
            if (this.authServiceConfig) {
                const tokenResponse = await this.tokenHandler.performTokenRequest(this.authServiceConfig, request)
    ...
    public revoke = (token: string) => {
        if (this.authServiceConfig) {
            this.tokenHandler.performRevokeTokenRequest(this.authServiceConfig, new RevokeTokenRequest({ token, client_id: this.config.client_id, token_type_hint: 'access_token' }))
        }
    }

    public getToken = async (): Promise<TokenResponse> => {
        this.authServiceConfig = await AuthorizationServiceConfiguration.fetchFromIssuer(this.config.issuer, this.requestor)
    ...
    signin = async () => {
        ...
        if (!this.token) {
            throw new Error("No token set after sign in")
        }
    }
    ...
    registerAxiosInterceptor = (instance: AxiosInstance): AxiosInstance => {
        instance.interceptors.request.use(async (config) => {
            if (!this.token) {
                console.log("No token in axios interceptor")
                await this.signin()
            } else {
                config.headers = {
                    ...config.headers,
